...
def main(args):
    ...
    if args.policy_mode == "local":
        policy = setup_policy(args)
    elif args.policy_mode == "websocket":
        ...
        for episode_idx in range(args.num_trials_per_task):
            ...
            for t in range(max_steps + args.num_steps_wait):
                ...
                if args.policy_mode == "websocket":
                    policy_img = image_tools.convert_to_uint8(
                        image_tools.resize_with_pad(policy_img, args.resize_size, args.resize_size)
                    )
                    policy_wrist_img = image_tools.convert_to_uint8(
                        image_tools.resize_with_pad(policy_wrist_img, args.resize_size, args.resize_size)
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Policy backend. Use local for RLinf checkpoints, or websocket to evaluate an already-running official OpenPI policy server.",
    )
    parser.add_argument(
        "--host",
        type=str,
        default="0.0.0.0",
        help="Host for websocket policy mode.",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=8000,
        help="Port for websocket policy mode.",
    )
    parser.add_argument(
        "--resize_size",
        type=int,
        default=224,
        help="Image resize size for websocket policy mode, matching the official OpenPI LIBERO client.",
    )
    parser.add_argument(
        "--task_suite_name",
        type=str,
