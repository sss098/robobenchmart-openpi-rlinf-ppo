...
EOF
    ...
    local openpi_transformers_replace_dir
    openpi_transformers_replace_dir=$(python - <<EOF
from pathlib import Path
import openpi

print(Path(openpi.__file__).resolve().parent / "models_pytorch" / "transformers_replace")
EOF
)
    if [ ! -d "$openpi_transformers_replace_dir" ]; then
        echo "OpenPI transformers_replace directory not found: $openpi_transformers_replace_dir" >&2
        exit 1
    fi
    cp -r "$openpi_transformers_replace_dir/"* \
...
install_openpi_package() {
    local openpi_dir
    openpi_dir="$(printenv OPENPI_PATH 2>/dev/null || true)"
    if [ -z "$openpi_dir" ] && [ -d "$SCRIPT_DIR/../../openpi" ]; then
        openpi_dir="$SCRIPT_DIR/../../openpi"
    fi

    if [ -n "$openpi_dir" ]; then
        if [ ! -d "$openpi_dir" ]; then
            echo "OPENPI_PATH is set to '$openpi_dir' but the directory does not exist." >&2
            exit 1
        fi
        uv pip install -e "$(realpath "$openpi_dir")"
    else
...
install_maniskill_package() {
    local maniskill_dir
    maniskill_dir="$(printenv MANISKILL_PATH 2>/dev/null || true)"
    if [ -z "$maniskill_dir" ] && [ -d "$SCRIPT_DIR/../../ManiSkill-3.0.0b22" ]; then
        maniskill_dir="$SCRIPT_DIR/../../ManiSkill-3.0.0b22"
    fi

    if [ -n "$maniskill_dir" ]; then
        if [ ! -d "$maniskill_dir" ]; then
            echo "MANISKILL_PATH is set to '$maniskill_dir' but the directory does not exist." >&2
            exit 1
        fi
        uv pip install -e "$(realpath "$maniskill_dir")"
    else
