...
def prepare_actions_for_robobenchmart(
    raw_chunk_actions,
    action_dim,
) -> np.ndarray:
    chunk_actions = np.asarray(raw_chunk_actions, dtype=np.float32)[..., :action_dim].copy()
    # Match RoboBenchMart's official eval_policy_client.py for ds_fetch_basket.
    # These two controlled joints are held fixed during evaluation/data collection;
    # leaving model outputs here can make the robot collide with shelf objects.
    if chunk_actions.shape[-1] > 9:
        chunk_actions[..., 8] = 0.0
        chunk_actions[..., 9] = 0.0
    return chunk_actions
