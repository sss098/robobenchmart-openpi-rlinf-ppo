...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_rbm",
        # RoboBenchMart pi0.5 SFT/eval uses the Pi0Config default horizon=50.
        # RLinf can still execute a shorter chunk via actor.model.openpi.action_chunk.
        model=pi0_config.Pi0Config(
        ...
        data=LeRobotRoboBenchMartDataConfig(
            repo_id="rbm_dataset",
            base_config=DataConfig(prompt_from_task=True),
            assets=AssetsConfig(assets_dir="checkpoints/torch/pi05_rbm/assets"),
            extra_delta_transform=False,
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader(
            "checkpoints/jax/pi05_base"
