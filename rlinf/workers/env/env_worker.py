...
class EnvWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self._action_trace_counters = defaultdict(int)
    ...
    def env_evaluate_step(
        ...
        stage_id: int,
        force_metrics_for_active: bool = False,
    ) -> tuple[EnvOutput, dict[str, Any]]:
        """
        ...
        self._record_action_trace(self.cfg.env.eval, "eval", stage_id, chunk_actions)
        env_info = {}
        ...
        if force_metrics_for_active and "episode" in infos:
            if self.cfg.env.eval.auto_reset:
                active = ~newly_done
            else:
                active = ~self.eval_prev_done[stage_id].to(current_dones.device)
            if active.any():
                for key in infos["episode"]:
                    active_value = infos["episode"][key][active].cpu()
                    if key in env_info:
                        env_info[key] = torch.cat([env_info[key], active_value], dim=0)
                    else:
                        env_info[key] = active_value
    ...
    def _record_action_trace(self, env_cfg, mode: str, stage_id: int, chunk_actions):
        trace_dir = env_cfg.get("action_trace_dir", None)
        if not trace_dir:
            return

        os.makedirs(trace_dir, exist_ok=True)
        key = (mode, stage_id)
        trace_idx = self._action_trace_counters[key]
        self._action_trace_counters[key] += 1
        ...
        out_path = os.path.join(
            trace_dir,
            f"{mode}_stage{stage_id}_rank{self._rank}_chunk{trace_idx:06d}.npz",
        )
        np.savez_compressed(
            out_path,
            ...
            chunk_idx=np.asarray(trace_idx, dtype=np.int64),
        )
    ...
    def evaluate(self, input_channel: Channel, rollout_channel: Channel):
            ...
            for eval_step in range(self.n_eval_chunk_steps):
                for stage_id in range(self.stage_num):
                    ...
                    env_output, env_info = self.env_evaluate_step(
                        raw_chunk_actions,
                        stage_id,
                        force_metrics_for_active=force_metrics_for_active,
                    )
        ...
        for stage_id in range(self.stage_num):
            if self.cfg.env.eval.video_cfg.save_video and isinstance(
                self.eval_env_list[stage_id], RecordVideo
            ):
                self.eval_env_list[stage_id].flush_video()
                self.eval_env_list[stage_id].close()
            elif self.cfg.env.eval.get("enable_offload", False) and hasattr(
