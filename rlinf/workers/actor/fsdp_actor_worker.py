...
class EmbodiedFSDPActor(FSDPModelManager, Worker):
    ...
    def init_worker(self) -> None:
        ...
        if self.kl_beta > 0 and self.combine_reference_model:
            self.ref_policy_state_dict = retrieve_model_state_dict_in_cpu(self.model)
            self.offload_model_buffer = {}
    ...
    def _compute_embodied_ref_logprobs(self, forward_inputs: dict[str, torch.Tensor]):
        ...
        try:
            ...
            if was_training:
                self.model.train()
        return ref_output["logprobs"].detach()

    def _compute_embodied_ref_kl_loss(
        self,
        ...
        elif self.cfg.algorithm.logprob_type == "action_level":
                    ...
                    if self.kl_beta > 0:
                        ...
                        if ref_logprobs is None:
                            raise RuntimeError(
                                ...
                                "reference policy state. Set actor.combine_reference_model=True "
                                "or algorithm.kl_beta=0."
                            )
